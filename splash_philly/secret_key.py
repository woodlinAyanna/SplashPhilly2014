...
def generate_or_read_from_file(key_file='.secret_key', key_length=64,skip_permission_check=False):
...
"""
    ...
    with lock:
        if not os.path.exists(key_file):
            ...
            if not skip_permission_check and oct(os.stat(key_file).st_mode & 0o777) != '0600':
                raise FilePermissionError("Insecure key file permissions!" + oct(os.stat(key_file).st_mode & 0o777))
...
def get_google_api_key(key_file='.google_api_key'):
    if not os.path.exists(key_file):
        raise FileNotFoundError("Google API Key not found.")
    else:
        with open(key_file, 'r') as f:
            key = f.readline()
        return key
